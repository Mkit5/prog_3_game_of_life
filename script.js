...
function setup() {
  ...
  //   let wallX = Math.floor( evt.pageX/ side)
  //   let wallY = Math.floor(evt.pageY / side)
...
// socket.emit("wall", click(evt))

function mouseClicked() {
  let wallX = Math.floor(mouseX / 10);
  let wallY = Math.floor(mouseY / 10);
  console.log(wallX, wallY);
  let sendCoords = {
    wallX: wallX,
    wallY: wallY,
  };
  socket.emit("wall", sendCoords);
}
